...
{
    // PORT return number of milliseconds since board init
}
...
uint8_t VL53L1X_getAddress(VL53L1X* vl) { return vl->address; }
...
DistanceMode VL53L1X_getDistanceMode(VL53L1X* vl) { return vl->distance_mode; }
...
uint16_t VL53L1X_readRangeContinuousMillimeters(VL53L1X* vl, int blocking) { return read(vl, blocking); } // alias of read(), default 1 = true for blocking

// check if sensor has new reading available
// assumes interrupt is active low (GPIO_HV_MUX__CTRL bit 4 is 1)
int VL53L1X_dataReady(VL53L1X* vl) { return (readReg(vl, GPIO__TIO_HV_STATUS) & 0x01) == 0; } // was returning bool

static const char * VL53L1X_rangeStatusToString(VL53L1X* vl, RangeStatus status);

void VL53L1X_setTimeout(VL53L1X* vl, uint16_t timeout) { vl->io_timeout = timeout; }
uint16_t VL53L1X_getTimeout(VL53L1X* vl) { return vl->io_timeout; }
int VL53L1X_timeoutOccurred(VL53L1X* vl); // was returning obol

//=========PRIVATE=========

// Record the current time to check an upcoming timeout against
void VL53L1X_startTimeout(VL53L1X* vl) { vl->timeout_start_ms = millis(); }

// Check if timeout is enabled (set to nonzero value) and has expired
int VL53L1X_checkTimeoutExpired(VL53L1X* vl) {return (vl->io_timeout > 0) && ((uint16_t)(millis() - vl->timeout_start_ms) > vl->io_timeout); }
...
float VL53L1X_countRateFixedToFloat(VL53L1X* vl, uint16_t count_rate_fixed) { return (float)count_rate_fixed / (1 << 7); }
