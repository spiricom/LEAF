...
void remove_transient(transient_pool* const po, unsigned int id)
{
	_transient_pool* pool = *po;
	int i;
    _transient *n;
...
Lfloat   tVoc_tick         (tVoc* const voc)
{
	...
	if(v->counter == 0) {
		...
		for(i = 0; i < 512; i++) {
			vocal_output = 0.0f;
			lambda1 = (Lfloat) i / 512.0f;
			lambda2 = (Lfloat) (i + 0.5f) / 512.0f;
			glot = glottis_compute(&v->glot, lambda1);
...
Lfloat * tVoc_get_frequency_ptr(tVoc* const voc)
{
	_tVoc* v = *voc;
	return &v->glot->freq;
}
